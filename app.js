...
function detectTapeBand(canvas){
  ...
  let peak=0,xc=0;for(let i=0;i<sw;i++)if(sm[i]>peak){peak=sm[i];xc=i;}
  if(peak<0.30)return null;
  let lo=xc;while(lo>0&&sm[lo]>0.5*peak)lo--;
  let hi=xc;while(hi<sw-1&&sm[hi]>0.5*peak)hi++;
  const margin=Math.round((hi-lo)*0.25)+Math.round(sw*0.008);
  lo=Math.max(0,lo-margin);hi=Math.min(sw-1,hi+margin);
  if((hi-lo)/sw>0.45)return null;                          // implausibly wide → not a tape
  const scale=w/sw;
...
function estimate(A,B){
  ...
  const m=matchMutual(A,B); R.matches=m.length;
  let inl=ransacIn(A,B,m,3,2000);
  if(!inl) return R;
  let Ms=fitSimilarity(inl.P,inl.Q);
  if(!Ms) return R;
  const gm=matchGuided(A,B,Ms,14);
...
function registerPair(A,B,lockScale){
  const e=estimate(A,B);
  const plausible = e.model && e.inliers>=8 && Math.abs(e.rot)<12 && e.scale>0.6 && e.scale<1.6;
  if(plausible){return {model:lockScale?deScale(e.model):e.model,affine:e.affine,inliers:e.inliers,matches:e.matches,quality:"firm"};}
  if(e.matches>=6){return {model:medianTranslation(A,B,matchMutual(A,B)),inliers:e.inliers,matches:e.matches,quality:"coarse"};}
...
function rawAffine(A,B){ return estimate(A,B); }
...
function analyzeFrames(F){
  ...
  for(let iter=0; iter<n; iter++){
    ...
    const seam=[]; for(let k=0;k<kl.length-1;k++) seam.push(rawAffine(F[kl[k]].feat, F[kl[k+1]].feat));
    ...
    for(let k=1;k<kl.length-1;k++){
      ...
      const rab=seam[k-1], rbc=seam[k], rac=rawAffine(F[a].feat,F[c].feat);
  ...
  if(kl.length>=2){
    ...
    if(!F[b].forceKeep && isDuplicate(rawAffine(F[a].feat,F[b].feat),F[0].dw,dh)){
...
async function run(){
  ...
  try{
    ...
    for(let i=0;i<F.length-1;i++){
      let r=registerPair(F[i].feat,F[i+1].feat,lock);
      if(r.quality!=="firm" && engine!=="xfeat"){  // ORB: escalate with a bigger feature budget.
        ...
        await tick();
        const r2=registerPair(featHi(F[i]),featHi(F[i+1]),lock);
      ...
      S.seams.push({i,M,inliers:r.inliers,matches:r.matches,quality:q,refined:ref>=0,affine:!!r.affine,ux:0,uy:0});
    ...
    if(weak) banner("warn",`${exNote}${weak} seam${weak>1?"s":""} couldn’t match reliably and were stacked straight — check the flagged seams and nudge.`);
...
function enforceConsistency(seams,F){
  ...
  const mags=dys.map(Math.abs).filter(v=>v>1).sort((a,b)=>a-b);
  const medMag=mags.length?mags[mags.length>>1]:Math.round(F[0].h*0.5);
  seams.forEach((s)=>{
    const dy=s.M[5], adv=dy*domSign; // advance in dominant direction (should be >0)
    const hi=F[s.i].h*0.95, lo=F[s.i].h*0.10;
    const bad = adv<lo || Math.abs(dy)>hi || (Math.abs(dy)>2.2*medMag) || (Math.abs(dy)<0.35*medMag && s.quality!=="firm");
    if(bad){
      s.M=[1,0,0, 0,1, domSign*medMag]; // safe stack in the right direction, median step
      s.quality="stack"; s.repaired=true;
    }
  });
  return seams;
}
...
function composite(){
  ...
  // bbox
  let minX=1e9,minY=1e9,maxX=-1e9,maxY=-1e9,cyc=[];
  F.forEach((f,i)=>{const M=T[i];let sy=0,ny=0;
    [[0,0],[f.w,0],[f.w,f.h],[0,f.h]].forEach(([x,y])=>{const p=apply(M,x,y);
      minX=Math.min(minX,p.x);minY=Math.min(minY,p.y);maxX=Math.max(maxX,p.x);maxY=Math.max(maxY,p.y);});
    const c=apply(M,f.w/2,f.h/2);cyc.push(c.y);
  });
  ...
  F.forEach((f,i)=>{
    // exposure gain vs previous frame from mean luminance of the canvas
    let gain=1;
...
function showResult(base){
  ...
  box.innerHTML=`<div class="rmeta"><span>output ${base.width}×${base.height}px</span><span>${S.imgs.length} frames · ${S.seams.length} seams</span></div>
    <div class="viewer"></div><a class="dl" id="dl" download="section.${big?"jpg":"png"}" href="#">⬇ Download ${big?"JPEG":"PNG"}</a>`;
...
function renderSeams(){
  ...
  S.seams.forEach((s,k)=>{
    let cls,label;
    if(s.quality==="stack"){cls="low";label="stacked";}
    else if(s.quality==="coarse"){cls="mid";label=`~${s.inliers} inl`;}
    else {cls=s.inliers>=25?"ok":"mid";label=`${s.inliers} inl${s.refined?" ✓":""}${s.affine?" tilt":""}`;}
    const el=document.createElement("div");el.className="seam";
    el.innerHTML=`<span class="join">${String(k+1).padStart(2,"0")}→${String(k+2).padStart(2,"0")}${s.quality==="stack"?" · fallback":s.quality==="coarse"?" · coarse":""}</span>
