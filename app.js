...
function estimate(A,B,minDy){
  const R={model:null,affine:false,inliers:0,matches:0,scale:0,rot:0,dx:0,dy:0};
  let m=matchMutual(A,B);
  // minDy: consider only matches that actually advance vertically (kills the entire family
  // of small-step self-similar correspondences — ruler ticks, tape digits, repeated lichen —
  // in one pass, instead of knocking out one consensus at a time)
  if(minDy>0)m=m.filter(([i,j])=>Math.abs(A.pts[i].y-B.pts[j].y)>=minDy);
  ...
  if(minDy>0)gm=gm.filter(([i,j])=>Math.abs(A.pts[i].y-B.pts[j].y)>=minDy);
...
function registerPair(A,B,lockScale,minAdv){
  const e=estimateRobust(A,B,minAdv);
  if(plausibleE(e)){return {model:lockScale?deScale(e.model):e.model,affine:e.affine,inliers:e.inliers,matches:e.matches,quality:"firm",aliased:e.aliased||0};}
  if(e.matches>=6){return {model:medianTranslation(A,B,e.m||matchMutual(A,B)),inliers:e.inliers,matches:e.matches,quality:"coarse"};}
...
async function run(){
  ...
  try{
    ...
    for(let i=0;i<F.length-1;i++){
      ...
      let r=registerPair(F[i].feat,F[i+1].feat,lock,minAdv);
      ...
      S.seams.push({i,M,inliers:r.inliers,matches:r.matches,quality:q,refined:ref>=0,affine:!!r.affine,aliased:r.aliased||0,ux:0,uy:0});
...
function renderSeams(){
  ...
  S.seams.forEach((s,k)=>{
    ...
    else {cls=s.suspect?"mid":(s.inliers>=25?"ok":"mid");label=`${s.inliers} inl${s.refined?" ✓":""}${s.affine?" tilt":""}${s.aliased?" ≠ruler":""}`;}
